...
const IntroView: FC<{ setUserAsVisited: Dispatch<void> }> = ({ setUserAsVisited }) => {
  return (
    <div
      className="relative flex h-[86vh] w-full flex-col-reverse items-center
    ...
    >
      <div className="left-0 my-10 flex size-full flex-col items-start justify-end sm:mt-0 md:mb-0 md:justify-center">
        <div
          className="ml-0 flex w-4/5 flex-col justify-center gap-16
        sm:ml-[20px] sm:w-3/5 md:w-[50%] lg:ml-[100px] lg:w-[35%] xl:ml-[200px]"
        >
          <TextBlock
            mainText="Use the"
            highlightText="Stars"
...
const ActionButton: FC<{ onClick: () => void, text: string }> = ({ onClick, text }) => (
  <button onClick={onClick} className="mt-10 w-full rounded-lg bg-primary px-8 py-3 text-xl font-bold text-white sm:w-[70%]">
    {text}
  </button>
);
