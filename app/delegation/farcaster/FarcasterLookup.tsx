...
export const FarcasterLookup: React.FC<Props> = ({ categoryName, handleDelegate }) => {
  ...
  const onDelegate = () => {
    if (!isValid) return;
    handleDelegate(extractFarcasterUsername(username), {
  ...
  return (
    <div className="mx-auto flex max-w-[500px] flex-col items-center justify-center gap-2 rounded-lg bg-white px-6 py-8">
      <div className="flex flex-col items-center justify-center">
        <Image src="/assets/images/warpcast2.png" alt="Farcaster icon" width={35} height={15} />
        <h2 className="my-2 text-xl font-medium">Delegate with Farcaster</h2>
      </div>
      <p className="mb-4 text-center text-gray-600">
        <span className="text-sm"> Delegating for </span>
        <br />
        <span className="font-bold text-red-500">{categoryName}</span>
      ...
      <div className="h-8 self-start text-sm">
        {isValid ? (
          ...
          <span className="text-primary">No user found with this username</span>
        ) : null}
      </div>
      <button
        onClick={onDelegate}
        className={`w-full rounded-md ${
          isValid && isValid.username !== connectionStatus?.farcaster?.metadata['username']
